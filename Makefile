...
# hilios.io/hilios-operator-bundle:$VERSION and hilios.io/hilios-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= hilios.io/hilios-operator
...
.PHONY: test-e2e  # Run KUTTL e2e tests against a Kind k8s instance.
test-e2e:
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name hilios-operator-builder
	$(CONTAINER_TOOL) buildx use hilios-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm hilios-operator-builder
	rm Dockerfile.cross
